...
public class AccountDetail extends Activity implements CreateNdefMessageCallback, OnNdefPushCompleteCallback{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		act_id = SelectedAccount.getString("act_id");
		com_id = SelectedAccount.getString("com_id");
	...
	public NdefMessage createNdefMessage(NfcEvent event) {
		...
		String text = client_id+"@"+ePWD+"@"+time+"@true@"+com_id+"@"+act_id;
	
		if (offer_act_id!=null && offer_com_id!=null)
			text = client_id+"@"+ePWD+"@"+time+"@"+com_id+"@"+act_id+"@"+offer_com_id+"@"+offer_act_id;
	...
	private class JsonReadTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			HttpPost httppost = new HttpPost(params[0]+"?record_client_id="+client_id+"&epw="+ePWD+"&time="+time+"&com_id="+com_id+"&act_id="+act_id+"&BASE64=1");
			System.out.println(params[0]+"?record_client_id="+client_id+"&epw="+ePWD+"&time="+time+"&com_id="+com_id+"&act_id="+act_id+"&BASE64=1");
			try {
	...
	public void ListDrwaer() {
		...
		try {
			...
			for (int i = 0; i < jsonMainNode.length(); i++) {
				...
				if (com_id.equals(record_com_id))
				{
					...
					for (int j = 0; j < jsonCard.length(); j++) {
						...
						if(record_idaa.equals(record_id)){
							...
							if (act_id.equals(record_act_id)){
    ...
    protected void onRestart() {  
		...
		gotoMenu.setClass(AccountDetail.this, Passcode.class);
