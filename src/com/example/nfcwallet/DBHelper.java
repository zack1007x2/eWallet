...
public class DBHelper extends SQLiteOpenHelper {
	...
	private static final String CREATE_NEWREPLAYS_TABLE = "CREATE TABLE NEWreplays(_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, transactions BLOB, type INTEGER, built_in INTEGER DEFAULT 0)";
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL(CREATE_NEWREPLAYS_TABLE);
		...
		saveTransactions(db, "Visa", vivoPayVisa, REPLAY_PCD, 1);			
		saveTransactions(db, "MasterCard", vivoPayMasterCard, REPLAY_PCD, 1);
		saveTransactions(db, "Discover", vivoPayDiscover, REPLAY_PCD, 1);
		saveTransactions(db, "AmericanExpress", vivoPayAmex, REPLAY_PCD, 1);
	}
	...
	private long saveNEWTransactions(SQLiteDatabase db,String name, byte[][] transactions, int type, int builtIn) {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		ObjectOutputStream oos;
		try {
			oos = new ObjectOutputStream(baos);
		...
		byte[] tBytes = baos.toByteArray();
        SQLiteStatement stmt = db.compileStatement("INSERT OR IGNORE INTO NEWreplays (name, transactions , type, built_in) values (?, ?, ?, ?)");
        stmt.bindString(1, name);
        stmt.bindBlob(2, tBytes);
        stmt.bindLong(3, type);
        stmt.bindLong(4, builtIn);
	...
	public long saveNEWTransactions(String name, byte[][] transactions, int type) {
		SQLiteDatabase wNEWDB = getWritableDatabase();
		return saveNEWTransactions(wNEWDB, name, transactions, type, 0);
	}
	
	public Cursor getNEWReplays() {
		  SQLiteDatabase rNEWDB = getReadableDatabase();
		  return rNEWDB.query("NEWreplays", null, null, null, null, null, null);
	}
	
	public int deleteNEWReplay(String name) {
		SQLiteDatabase wNEWDB = getWritableDatabase();
		SQLiteStatement stmt = wNEWDB.compileStatement("DELETE FROM NEWreplays WHERE name = ? AND built_in = 0");
		stmt.bindString(1, name);
		return stmt.executeUpdateDelete();
	}
