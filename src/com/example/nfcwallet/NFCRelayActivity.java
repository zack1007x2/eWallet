...
//	}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mTabHost.setup();
		mTabHost.addTab(mTabHost.newTabSpec("status_tab")
				.setContent(R.id.statusTab)
				.setIndicator(getString(R.string.status)));
		...
		if (mServerSocket == null
				|| (mServerSocket != null && mServerSocket.isBound() == false)) {
			log("Starting Server...");
		...
		try {
			...
			if (net != null) {
				for (Enumeration<InetAddress> enumIpAddr = net
	...
	public void updateUIandScroll(CharSequence msg) {
		...
		// use post so that recent update from setText/append takes effect first
		// (at least a better chance of updating)
	...
	public void onResume() {
		...
		SharedPreferences prefs = getSharedPreferences(NFCVars.PREFERENCES,
				Context.MODE_PRIVATE);
		if (!prefs.getBoolean("relayPref", false)) {
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
				prefs.edit().putBoolean("relayPref", true).commit();
				Toast.makeText(this,
						getString(R.string.proxy_mode_unsupported),
		...
		if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction())) {
			Toast.makeText(this, getString(R.string.ndef_discovered),
					Toast.LENGTH_SHORT).show();
			// TODO
		} else if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
			Toast.makeText(this, getString(R.string.tech_discovered),
					Toast.LENGTH_SHORT).show();
			// TODO
		} else if (NfcAdapter.ACTION_TAG_DISCOVERED.equals(intent.getAction())) {
			Toast.makeText(this, getString(R.string.tag_discovered),
					Toast.LENGTH_SHORT).show();
	...
	private String getTagInfo(Intent intent) {
		...
		Parcelable[] extraNdefMsg = intent
				.getParcelableArrayExtra(NfcAdapter.EXTRA_NDEF_MESSAGES); // optional
		...
		public void run() {
			...
			while (true) {
				...
				try {
					...
					if (mTagTech != null
							&& (clientSocket != null && clientSocket
									.isConnected())) {
						...
						if (line.equals(NFCVars.READY)) {
							...
							if (line.equals(NFCVars.ENCRYPT)) {
								...
								if (mEncrypt) {
									// Have to do this every time...TODO: notify
									// ends upon password change?
									mSecret = generateSecretKey(salt);
								}

								IOUtils.sendSocket(
										NFCVars.VERIFY.getBytes("UTF-8"), os,
										mSecret, mEncrypt);

								byte[] ok = IOUtils.readSocket(is, mSecret,
										mEncrypt);
								if (ok == null) {
						...
						try {
							...
							// From IsoPcdA doc
							// @param data - on the first call to transceive
							// after PCD activation, the data sent to the method
							// will be ignored
							IOUtils.sendSocket(mTagTech.getTag().getId(), os,
									mSecret, mEncrypt);

							byte[] response = null;
							do {
								response = IOUtils.readSocket(is, mSecret,
										mEncrypt);
								if (response == null) {
									log("no response from PCD");
									break;
								}
								log("response from PCD: "
										+ TextHelper
												.byteArrayToHexString(response));

								log("sending response to card");
								response = mTagTech.transceive(response);
								log("response from card: "
										+ TextHelper
												.byteArrayToHexString(response));

								log("sending card response to PCD");

								IOUtils.sendSocket(response, os, mSecret,
										mEncrypt);
							...
							if (clientSocket != null) {
								try {
									if (!clientSocket.isClosed()) {
										...
										IOUtils.sendSocket("Relay lost tag"
												.getBytes("UTF-8"),
												clientSocket.getOutputStream(),
												mSecret, mEncrypt);
									}
							...
							if (e.getCause() instanceof IllegalBlockSizeException) {
								...
								if (clientSocket != null) {
									try {
										// if
										// (e.getMessage().contains("Transceive failed")
						...
						if (mTagTech == null) {
							...
							if (clientSocket != null) {
								...
								// os.flush();
								// sendEncrypted("NOT READY".getBytes("UTF-8"),
								// clientSocket.getOutputStream(), salt);
								IOUtils.sendSocket(
										"NOT READY\n".getBytes("UTF-8"),
	...
	private SecretKey generateSecretKey(byte[] salt) throws IOException {
		try {
			...
			KeySpec ks = new PBEKeySpec(prefs.getString("passwordPref",
					getString(R.string.default_password)).toCharArray(), salt,
					2000, 256);
